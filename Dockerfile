...
ARG BUILD_FROM=ghcr.io/imagegenius/immich:v2.5.6-ig447
...
ENV \
    ...
    NGINX_PORT=8080 \
    SERVER_PORT=2283 \
    DB_URL="postgresql:///immich?host=/var/run/postgresql&user=immich&password=immich" \
    REDIS_SOCKET="/var/run/redis/redis.sock" \
    MACHINE_LEARNING_CACHE_FOLDER="/data/cache" \
...
ENV \
    ...
    POSTGRES_SHARED_PRELOAD_LIBRARIES="vchord.so, vectors.so" \
...
RUN set -eux; \
    if [ -f /etc/dpkg/dpkg.cfg.d/docker ]; then \
        ...
        grep -q '/usr/share/locale' /etc/dpkg/dpkg.cfg.d/docker && exit 1; \
    fi; \
    apt-get update; \
...
RUN set -eux; \
    ...
    apt-get install -y --no-install-recommends /tmp/pgvectors.deb; \
